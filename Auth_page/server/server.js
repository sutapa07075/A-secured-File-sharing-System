...
passport.deserializeUser(async (id, done) => {
  try {
    const user = await db.get('SELECT id, email, name, provider, avatar_url FROM users WHERE id = ?', [id]);
      ...
      if (!user) {
        ...
        if (user) {
          ...
          const info = await db.run(
            ...
            [email, profile.displayName, 'google', profile.id, avatarUrl]
          );
    ...
    try {
      ...
      if (!user) {
        ...
        if (user) {
          ...
          const info = await db.run(
            'INSERT INTO users (email, name, provider, provider_id, avatar_url) VALUES (?, ?, ?, ?, ?)',
            [email, profile.displayName || profile.username, 'github', profile.id, avatarUrl]
          );
          user = await db.get('SELECT * FROM users WHERE id = ?', [info.lastID]);
        }
      }
...
app.post('/api/register', authLimiter, async (req, res) => {
  try {
    ...
    const info = await db.run(
      ...
      [email.toLowerCase(), hash, name || null, 'local']
    );

    const user = await db.get('SELECT id, email, name, provider FROM users WHERE id = ?', [info.lastID]);
...
app.post('/api/login', authLimiter, async (req, res) => {
  try {
    ...
    const safeUser = { id: user.id, email: user.email, name: user.name, provider: user.provider };
